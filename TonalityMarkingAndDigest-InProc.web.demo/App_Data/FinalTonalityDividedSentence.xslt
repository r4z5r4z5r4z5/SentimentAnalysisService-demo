...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
            ...
            <xsl:when test="@TYPE = 'Inquiry'">
              <xsl:attribute name="style">
                <xsl:if test="@SNTFINAL">
                  <xsl:call-template name="SNTFINAL-2-fore-color" />
                  <xsl:call-template name="FRTFINAL-2-font-size" />
                </xsl:if>
                <xsl:if test="@INQUIRYINITIAL = 'TRUE'">
  ...
  <xsl:template match="SENT[ node() ]">
    <hr class="sentence-hr" /><span class="sentence-number"><xsl:value-of select="position()" />]. </span>
    <xsl:apply-templates select="node()"/>
  </xsl:template>

  <!--Выведение FRTFINAL, если есть элементы <FRTFINAL>(FRTFINAL: #)</FRTFINAL>-->
  <xsl:template match="FRTFINAL[ node() ]">
  ...
  <xsl:template name="FRTFINAL-2-font-size">
  ...
  <xsl:template name="SNTFINAL-2-fore-color">
    <xsl:choose>
